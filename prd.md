...
- **Approval is terminal (all depths).** When the newest review/recheck record on a PR carries `verdict=APPROVE`, `identifyNextWorkflowStep` returns `{ step: null, stopReason: 'approved' }` and `decideReviewOnly` returns `approved: true`. No later step runs on that PR — no recheck, no fresh review — even when new commits are pushed after the approval. `watch` remembers approved PRs in-session (`approvedPRKeys`) so later events skip before any API call, and re-derives the same stop from comment history after a restart. The single exception is work that landed *after* the approval: a fix or conflict-resolve pushed past an APPROVE (possible only in a workflow whose fix step is not gated on the verdict) still gets its recheck. Forcing another pass is an explicit act — `crosscheck run <pr-url> --steps review`, which skips history detection entirely.
