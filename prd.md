...
- `conflict-resolve` is orthogonal to the review→fix→recheck depth ladder, so an override cannot list it. It passes through whenever the override permits code modification (i.e. includes `fix`: `review,fix` or `review,fix,recheck`) and is dropped for overrides without `fix` (`review` or `review,recheck`). The same rule gates the scan's `resolve` next-action (`applyRepoWorkflowOverrideToStatus`).
- **A conflicted PR reaches `conflict-resolve` at any point in its life, not just before the first review.** Being orthogonal to the ladder cuts both ways: a PR conflicts because the BASE branch moved, which changes nothing about the reviewed code and fires no webhook. `identifyNextWorkflowStep` takes an optional `mergeable` hint and returns the `conflict-resolve` step whenever it is `false`, ahead of the approval stop and the fix route — an approved PR that can no longer merge is the case that matters most. Only an explicit `false` counts (`null` = GitHub still computing). The hint is never authoritative: the step re-checks with its own `pulls.get` and a real merge probe and skips as `no_conflicts`, so a stale hint costs one cheap skip. One attempt per review round; the reported `round` counts conflict-resolve attempts rather than review rounds, so a PR deep into fix→recheck cycles does not lose conflict resolution to `max_rounds`. Because a base move produces no event, discovery belongs to the scan: `derivePRStatus` reports `nextAction: 'resolve'` for a conflicted PR regardless of review state, `kickass` dispatches it as a pinned `--steps conflict-resolve` leg (fork PRs skip, as with fix), and the picker sorts it first.
...
- **An approval is terminal for the commit it covers, not for the PR (all depths).** When the newest review/recheck record carries `verdict=APPROVE` *and* its `sha=` matches HEAD (prefix match, so short and long forms both work), `identifyNextWorkflowStep` returns `{ step: null, stopReason: 'approved' }` — no recheck, no re-review, for as long as HEAD stays there. A push moves HEAD off the approved SHA and materially changes what was approved, so the approval no longer applies: detection falls through to a fresh review at `lastReview.round + 1` and the normal fix/recheck loop resumes. A legacy `APPROVE` with no recorded `sha=` cannot prove it covers HEAD and is treated the same way (fail open — one review re-establishes the SHA and the stop). `watch` holds no session state that can answer this: it reads history on every event, including `issue_comment` triggers. Both session fast-paths were removed because approval state is not knowable from what one process did — `kickass`, `crosscheck run`, or a second watcher can post an APPROVE, force a re-review, or push a fix that this process never sees. Session state now serves one purpose only: `handledCommentIds` dedups webhook redeliveries of the same `issue_comment`. There is nothing to suppress beyond that — `webhook.ts` forwards only annotations carrying `trigger=kickass`, and this watcher's own reviews carry `trigger=watch`/`comment`/`backtrace`, so they never reach the bridge. Every comment that does reach it is a kickass dispatch whose verdict the watcher is meant to act on, and history detection decides how. When history cannot be read (after one retry), every path fails closed: the event is deferred with `reason: history_unavailable`, and the `crosscheck/review` commit status is released as **`failure`**, not `success` — it is a status a repo can require for merge, and a green one on a HEAD nobody reviewed would let an unreviewed commit satisfy branch protection during a GitHub outage. `auto_loop` is the exception — it continues a run this process just completed, so its state is known from the session, and deferring would strand the loop.

  The same SHA scoping applies to the scan: `derivePRStatus` maps an `APPROVE` to `APPROVED`/`nextAction: 'merge'` only when the annotation's `sha=` covers HEAD. The SHA is read from the newest verdict-bearing *annotation*, never from the merged latest verdict — that list also holds `review_complete`/`workflow_complete` log events, which carry no SHA and are written after the comment is posted, so the newest verdict on a freshly approved PR is normally SHA-less. Taking the SHA from there would fail every approval check and have kickass re-dispatch approved PRs forever. That check runs *before* the fix-log branch: fix log events carry no SHA, so a fix force-pushed away — or delivered as its own PR, leaving HEAD untouched — would otherwise pin an approved commit to `NEEDS_RECHECK` and have kickass dispatch no-op rechecks at it forever. Ordering it first is self-correcting, since a fix that really did move HEAD also moves it off the approved SHA. Without that, `kickass` would list a PR approved at an older commit under "needs merge (manual)" and never dispatch the fresh review it is owed. Only `APPROVE` is SHA-scoped there: a stale `NEEDS_WORK`/`BLOCK` still means the PR has unresolved findings, and `hasUsableCurrentHeadReview` separately demotes a fix to a review when the annotation covers an older SHA. All four comparisons — detection, review-only dedup, scan approval, kickass current-head check — route through the single `shaCovers` helper.

  The post-approval fix exception is scoped to the pushed commit, not to the record: it re-opens the workflow only while that fix/conflict-resolve commit is HEAD. A force-push that drops it and returns HEAD to the approved SHA leaves a record describing content that no longer exists, and the approval stands. Work that landed *after* the approval is finished rather than stopped, but which step runs depends on what was pushed: a **fix** commit at HEAD gets its recheck (`fixedCurrentSha`), while a **conflict-resolve** merge commit gets a fresh review. That asymmetry is deliberate — a merge brings in base content nobody reviewed, and a recheck is instructed to confirm the original findings and *not* flag new issues, so it is the wrong lens for it. Reaching either through `watch` requires a trigger: the webhook path skips synchronize events whose HEAD is a `[crosscheck]` commit (the guard that stops the fix→review loop re-entering on our own pushes), so an out-of-band `crosscheck fix`/`resolve` is picked up by `crosscheck run`, `kickass`, or the next human push rather than by the commit itself. `review`-only depth needs no separate gate: the approved SHA is by definition an already-reviewed SHA (`decideReviewOnly`). Forcing a pass on the approved commit itself is an explicit act — `crosscheck run <pr-url> --steps review`, which skips history detection entirely.
