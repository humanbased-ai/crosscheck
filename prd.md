...
- `review,recheck` is a recheck-no-fix depth: crosscheck never auto-fixes, so a new (human-pushed) SHA routes to `recheck` — re-evaluating against the prior review — rather than a fresh review (`identifyNextWorkflowStep`). Because no fix step runs, the recheck step's default `when: "fix.applied_count > 0"` guard is cleared when the depth omits `fix` (`filterStepsByTypes`), otherwise the recheck would be skipped forever and the mode would silently degrade to review-only.
