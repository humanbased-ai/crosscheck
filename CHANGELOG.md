...
- **An auto-fix PR is closed when the PR it was cut from merges** ([#291](https://github.com/humanbased-ai/crosscheck/issues/291)). An auto-fix PR is a snapshot of the reviewed PR at the moment the fix ran, so once the source PR merges it holds changes against a tree that no longer exists. On `monorepo#2527` the author pushed their own fix for the same finding and merged; the auto-fix PR stayed open, GitHub retargeted it onto the merge commit, and merging it would have reintroduced exactly what the author's fix removed — while reading, on the PR page, like a routine cleanup merge. `watch` now handles the merged-PR webhook: it comments why the auto-fix PR is superseded (naming the source PR and its merge commit) and closes it. Only a PR carrying crosscheck's own auto-fix body is touched, so a hand-created branch of the same name is left alone, and a PR closed *unmerged* is not treated as a merge. The webhook sweep is a one-time lookup, so the fix step also re-checks the source PR's state immediately before pushing the fallback branch and skips the delivery (`source_pr_merged`) if it has merged since — otherwise a fix still in flight during the merge would open a stale PR the sweep had already passed by.
- **`conflict-resolve` no longer skips every PR crosscheck cannot attribute.** Three defects compounded into one silent no-op. (1) The default workflow gives `conflict-resolve` `reviewer: origin`; when origin detection returned `human` the step resolved no vendor and skipped with `no_vendor`, because only the fix step consulted `routing.fallback_reviewer` — it now applies to conflict-resolve too, resolving to a vendor that can actually run the step under `fallback_reviewer: auto`, and honouring an explicit `claude`/`codex` as written. (2) Origin detection did not recognise crosscheck's *own* output: its fix-PR body says `Fixed with [Claude Code]`, not `Generated with [Claude Code]`, and its commits carry `Crosscheck-Reviewer:` trailers rather than `Co-Authored-By`, so a PR crosscheck wrote detected as `human`. Both markers are now recognised **always**, independently of `routing.codex_reviews_patterns` / `claude_reviews_patterns` — setting either list replaces its defaults, so an install that pinned its own patterns would otherwise keep misreading crosscheck's own fix PRs as human. Configured patterns are still checked first and outrank it. Review attributions (`Reviewed with [...]`) and the `Attempted` footer of a failed fix are deliberately excluded: neither says anything about who wrote the code. (3) `crosscheck run` printed `✓ Workflow complete` whenever nothing threw, so a run where every step skipped looked identical to a successful one — it now prints `⚠ Workflow complete, no step ran` with each step's skip reason. Exit codes are unchanged. (4) A consequence of (2): once a crosscheck-authored Codex fix PR is correctly attributed to `codex`, `conflict-resolve` — which is Claude-only — would resolve to `codex` and skip as unsupported, leaving conflicts unresolved (before (2) these were `human` and the auto fallback picked Claude). When the vendor came from origin detection (`reviewer: origin`), the step now substitutes a capable enabled vendor instead of skipping, logging `conflict_resolve_vendor_fallback reason=unsupported_vendor`. An explicit `reviewer: claude|codex`, `reviewer: auto`, or `routing.fallback_reviewer` is still honoured as written.
