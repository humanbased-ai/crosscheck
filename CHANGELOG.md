...
- **Agent Skills now actually run under Codex.** They never had: `~/.crosscheck/logs` shows codex with **0 skill activations across 244 runs** while claude logged 131 on the same workflows, which is why codex-reviewed PRs never carried a `_Skills: …_` line in the comment footer. Two independent causes, both verified against codex-cli 0.147 with a probe MCP server that logs every JSON-RPC message it receives. (1) `codex review` starts no MCP servers at all — not those passed with `-c`, not those in `~/.codex/config.toml` — so the skill broker was unreachable on every review and the prompt's obligatory "Call `list_enabled_skills`" named a tool that did not exist in the session. **Codex reviews now run through `codex exec`**, which accepts both a prompt and MCP config; the prompt is built block-for-block like Claude's, passed on stdin so repository guidance and issue context stay out of the process list, and the review is read from `--output-last-message` so session chatter cannot reach the posted comment. The temporary Codex profile is retired — nothing is written to `$CODEX_HOME` any more. Because `exec` does not scope the diff the way `codex review --base` did, the prompt names the exact command — `git diff origin/<base>...HEAD` — rather than the base branch alone: the clone creates `refs/remotes/origin/<base>` and no local `<base>` branch, so the obvious reading of a bare branch name fails on any non-default base and can leave the agent reviewing an empty diff. (`codex exec review --base` is not the alternative it looks like: there `--base` and a custom prompt are mutually exclusive, so it cannot carry the verdict rule or the skill instructions.) (2) Where MCP servers *do* start, codex cancels every `tools/call` with "user cancelled MCP tool call" unless the sandbox is `danger-full-access` — `-a never`, `--full-auto`, `-s read-only`, `-s workspace-write`, `approval_policy="never"` and per-tool `approval_mode` all leave it cancelled. Codex skills therefore cost the codex sandbox on a step that reads untrusted PR content, so they are **opt-in and off by default**: the new `skills.codex_full_access` must be set before crosscheck will register the broker for codex or widen its sandbox. Left at its default, codex stays sandboxed and is simply told nothing about skills — registering a broker whose every call is cancelled is the original 0-activations failure, not a graceful fallback. Claude is unaffected; its broker works sandboxed. Three hardening changes apply to every codex run, granted full access or not: `codex exec` is invoked with `--ignore-user-config`, so a review of untrusted code no longer loads the operator's own MCP servers and plugins — `codex review` never read that file, so moving to `exec` had quietly widened what a prompt injection could reach; the subprocess environment is an **allowlist** — codex gets `PATH`, `HOME`, proxy and TLS settings and its own API key, and no longer inherits `GITHUB_TOKEN` or anything else the operator has exported; and the git credential embedded in the clone's `origin` URL is stripped for the duration of the agent run and restored afterwards, so an injected instruction cannot read a writable token out of `.git/config` or push with it.
